...
class ShFgwSpider(scrapy.Spider, BaseCrawl):
    name = 'sh_fgw'
    ...
    def parse(self, response):
        ...
        items = super(ShFgwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        url_xp = "./a"
        next_xp = ""
        super(ShFgwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def time_select(self):
        ...
        if self.part == 1:
            ...
            time.sleep(1)
            self.get_element_by_xpath("//a[@data-code='fgw.sh.gov.cn']").click()
            time.sleep(1)
            # 在《要闻动态》下搜索
        ...
        time.sleep(2)
        # 时间范围选择一年内
        self.get_element_by_xpath("//a[@id='drop6']","//div[@id='layui-layer-shade2']", "//div[@id='layui-layer-shade5']",
