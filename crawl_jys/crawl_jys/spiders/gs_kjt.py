...
class GsKjtSpider(scrapy.Spider, BaseCrawl):
    name = 'gs_kjt'
    start_urls = ['http://kjt.gansu.gov.cn']
    # custom_settings = {
    #     'HEADLESS': False,
    #     'IMAGELESS': True
    # }
    ...
    def parse(self, response):
        input_xpath = '//*[@id="word"]'
        search_xpath = "//input[@class='search_btn']"
        items = super(GsKjtSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(GsKjtSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '甘肃科技厅'  # 需要修改为当前的网站名，如：广东发改委
