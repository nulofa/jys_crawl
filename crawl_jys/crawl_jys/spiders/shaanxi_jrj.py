...
class ShaanxiJrjSpider(scrapy.Spider, BaseCrawl):
    name = 'shaanxi_jrj'
    ...
    def parse(self, response):
        ...
        items = super(ShaanxiJrjSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(ShaanxiJrjSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
                                               title_xp, url_xp, next_xp)

    def time_select(self):
        #selection department
        try:
            self.waitor('//div[@class="mainCon clearfix"]')
            self.get_element_by_xpath('//a[@id="codediv"]').click()
            self.waitor('//div[@id="codediv"]')
        except:
            time.sleep(2)
            self.get_element_by_xpath('//*[@id="codediv"]').click()
            self.waitor('//div[@id="codediv"]')
        self.get_element_by_xpath("//div[@id='codediv']/div[@id='coded"
                                  "iv']/div[@id='codediv'][2]/ul[@id='codediv']/li[@id='codediv'][10]/a").click()
        time.sleep(1)
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '陕西金融局'  # 需要修改为当前的网站名，如：广东发改委
    ...
    def process_date(self, new, date_xp):  # 返回[年，月，日]，如: 2021-12-12 则返回[2012,12,12]
        ...
        if len(new.find_elements_by_xpath(".//table")) > 0 or date_text.strip() == '':
            return ['1970', '1', '1']
