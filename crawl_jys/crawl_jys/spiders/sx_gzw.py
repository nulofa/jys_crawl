...
class SxGzwSpider(scrapy.Spider, BaseCrawl):
    name = 'sx_gzw'
    start_urls = ['http://gzw.shanxi.gov.cn']
    # custom_settings = {
    #     'HEADLESS': False,
    #     'IMAGELESS': True
    # }
    ...
    def parse(self, response):
        input_xpath = '//*[@id="keyvalue"]'
        search_xpath = "//input[@onclick='sysearchevent()']"
        items = super(SxGzwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(SxGzwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '山西国资委'  # 需要修改为当前的网站名，如：广东发改委
