...
class HbFgwSpider(scrapy.Spider, BaseCrawl):
    name = 'hb_fgw'
    start_urls = ['http://www.hubei.gov.cn']
    ...
    def parse(self, response):
        ...
        items = super(HbFgwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(HbFgwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def time_select(self):
        ...
        try:
            ...
            self.get_element_by_xpath("//div[@class='siteId_children department_site']//li[1]").click()
            self.get_element_by_xpath("//div[@class='siteIdCheckList']//div[@class='btns fr']/button[1]").click()
        except:
            if len(self.browser.find_elements_by_xpath("//div[@class='siteIdCheckList']//div[@class='btns fr']/button[2]"))>0:
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '湖北发改委'  # 需要修改为当前的网站名，如：广东发改委
