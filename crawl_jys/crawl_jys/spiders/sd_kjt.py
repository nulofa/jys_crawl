...
class SdKjtSpider(scrapy.Spider, BaseCrawl):
    name = 'sd_kjt'
    start_urls = ['http://kjt.shandong.gov.cn']
    # custom_settings = {
    #     'HEADLESS': False,
    #     'IMAGELESS': True
    # }
    ...
    def parse(self, response):
        input_xpath = "//input[@name='q']"
        search_xpath = '//*[@id="search"]'
        items = super(SdKjtSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(SdKjtSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def time_select(self):
        ...
        try:
            ...
            ActionChains(self.browser).move_to_element(self.get_element_by_xpath(time_xp)).perform()
            self.waitor(time_xp2)
        ActionChains(self.browser).click(self.browser.find_element_by_xpath(time_xp2)).perform()
        time.sleep(2)
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '山东科技厅'  # 需要修改为当前的网站名，如：广东发改委
