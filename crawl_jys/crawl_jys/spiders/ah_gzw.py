...
class AhGzwSpider(scrapy.Spider, BaseCrawl):
    name = 'ah_gzw'
    ...
    def parse(self, response):
        ...
        items = super(AhGzwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        next_xp = "//a[@title='下一页']"
        super(AhGzwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
                                          title_xp, url_xp, next_xp)

    def time_select(self):

        domain_xp = "//div[@class='module selected-container']"
        wait_xp = "//ul[@id='filllist']//a[@title='省国资委']"
