...
class ZjGzwSpider(scrapy.Spider, BaseCrawl):
    name = 'zj_gzw'
    start_urls = ['http://gzw.zj.gov.cn']
    ...
    def parse(self, response):
        input_xpath = "//input[@class='input_txt gt_input']"
        search_xpath = "//button[@class='gt_btn']"
        items = super(ZjGzwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(ZjGzwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def time_select(self):
        ...
        from_date = datetime.date.today() - datetime.timedelta(ZjGzwSpider.date_limit)
