...
class TjKjtSpider(scrapy.Spider, BaseCrawl):
    name = 'tj_kjt'
    start_urls = ['http://kxjs.tj.gov.cn']
    ...
    def parse(self, response):
        input_xpath = '//*[@id="key_wrd"]'
        search_xpath = "//input[@onclick='jumpToSearch()']"
        items = super(TjKjtSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(TjKjtSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '天津科技局'  #需要修改为当前的网站名，如：广东发改委
