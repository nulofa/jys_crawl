...
class HunanFgwSpider(scrapy.Spider, BaseCrawl):
    name = 'hunan_fgw'
    ...
    def parse(self, response):
        ...
        items = super(HunanFgwSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(HunanFgwSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
                                             title_xp, url_xp, next_xp)

    def time_select(self):
        #
        try:
            self.waitor("//div[@class='change-org fl']")
            self.get_element_by_xpath("//div[@class='change-org fl']").click()
            self.waitor("//div[@class='orgchange-con']")
            self.get_element_by_xpath("//div[@class='orgchange-con']//span[@data-id='30']").click() # fgw
            self.get_element_by_xpath("//div[@class='orgchange-org-btn']/a").click() # confirm
        except:
            time.sleep(2)
            self.waitor("//div[@class='change-org fl']")
            self.get_element_by_xpath("//div[@class='change-org fl']").click()
            self.waitor("//div[@class='orgchange-con']")
            self.get_element_by_xpath("//div[@class='orgchange-con']//span[@data-id='30']").click()
            self.get_element_by_xpath("//div[@class='orgchange-org-btn']/a").click()
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '湖南发改委'  # 需要修改为当前的网站名，如：广东发改委
