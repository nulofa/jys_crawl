...
class SxJrjSpider(scrapy.Spider, BaseCrawl):
    name = 'sx_jrj'
    start_urls = ['http://jrjgj.shanxi.gov.cn']
    # custom_settings = {
    #     'HEADLESS': False,
    #     'IMAGELESS': True
    # }
    ...
    def parse(self, response):
        ...
        search_xpath = '//*[@id="serch_btn"]'
        items = super(SxJrjSpider, self).myParse(response, input_xpath, search_xpath)
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        self.waitor(wait2_xp)
        self.get_element_by_xpath("//input[@class='searchInput']").send_keys(keyword)
        self.get_element_by_xpath("//input[@type='button']").click()
        super(SxJrjSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
    ...
    def process_item(self, new, item, title_xp, url_xp):
        ...
        item['source'] = '山西金融局'  # 需要修改为当前的网站名，如：广东发改委
