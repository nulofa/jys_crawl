...
class ScJrjSpider(scrapy.Spider, BaseCrawl):
    name = 'sc_jrj'
    start_urls = ['http://dfjrjgj.sc.gov.cn']
    ...
    def parse(self, response):
        input_xpath = '//*[@id="search_box"]'
        search_xpath = "//input[@id='search_btn']"
        items = super(ScJrjSpider, self).myParse(response, input_xpath, search_xpath, "//p[@class='fl']")
        for item in items:
            yield item
    ...
    def myGetData(self, keyword):
        ...
        super(ScJrjSpider, self).get_data(keyword, wait2_xp, wait3_xp, news_xp, date_xp, content_xp,
